...
/*
 * 功能:无限分类 使用样例： //测试数据 $data[]=array('cat_id'=>1,'pid'=>0,'name'=>'中国');
 ...
 * $data[]=array('cat_id'=>12,'pid'=>3,'name'=>'首尔'); $cat=new
 * Category(array('cat_id','pid','name','cname'));
 * $s=$cat->getTree($data);//获取分类数据树结构
 * //$s=$cat->getTree($data,1);获取pid=1所有子类数据树结构 foreach($s as $vo) { echo
 * $vo['cname'].'<br>'; } ?>
 */
class Category extends Base
{
     ...
     * 则，传递参数为,$field('cid','pid','name','cname');若为空，则采用默认值。 返回：无
     */
    ...
    private function _searchList ($id = 0, $space = "")
    {
        ...
        for ($i = 0; $i < $n; $i ++) {
            ...
            $childs[$i][$this->field['fulltitle']] = ($space ? $space . $pre : "") .
