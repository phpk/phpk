 ...
 *
 *
 * $db = new Backup('localhost','root','','guestbook','utf8','data/dbbak/');
 *
 * //查找数据库内所有数据表
...
class Backup
{
    ...
    public function getTables ($database = '')
    {
        ...
        $result = mysql_query("SHOW TABLES FROM `$database`") or
                 die(mysql_error());
    ...
    public function exportSql ($table = '', $subsection = 0)
    {
        ...
        if ($subsection == 0) {
            ...
            if (! is_array($table)) {
                ...
                $sqlArry = array();
                for ($i = 0; $i < count($table); $i ++) {
                    $tmpArry = $this->_setSql($table[$i], $subsection, 
                            $this->transfer);
            ...
            for ($i = 0; $i < count($sqlArry); $i ++) {
                $fileName = $this->dataDir . date("Ymd", time()) . '_part' . $i .
                         '.sql.php';
    ...
    /*
     * 载入sql文件，恢复数据库 @param diretory $dir @return booln
