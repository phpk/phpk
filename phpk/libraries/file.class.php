...
class File
{
    ...
    public static function create_file($file, $data ,$flags = null, $context = null , $storage = 'default')
    {
        ...
        if ( File::can_do_run($storage) )
        {
            ...
            {
                if (DEBUG > 0)debug('create file error:'.$file);
                return false;
            }
        }
        else
    ...
    protected static function call_http_host($storage, $uri, $arg1 = null, $arg2 = null)
    {
        ...
        $rs = call_user_func_array(array(factory('HttpCall',$storage), $action), $param_arr);
    ...
    {
        static $mode = null;
        if ( null === $mode ) $mode = C('core.file_sync_mode');
    ...
    {
        return (IS_SYSTEM_MODE || 'none' == File::sync_mode() ||count(C('core.web_server_list.' . $storage)) <= 1) ? true : false;
    }
