 ...
 *      $auth=new AuthRules();  $auth->getAuth('规则名称','用户id')
 * 2，可以同时对多条规则进行认证，并设置多条规则的关系（or或者and）
 *      $auth=new AuthRules();  $auth->getAuth('规则1,规则2','用户id','and') 
 *      第三个参数为and时表示，用户需要同时具有规则1和规则2的权限。 当第三个参数为or时，表示用户值需要具备其中一个条件即可。默认为or
 * 3，一个用户可以属于多个用户组(tb_auth_group_access表 定义了用户所属用户组)。我们需要设置每个用户组拥有哪些规则(tb_auth_group 定义了用户组权限)
 * 
 * 4，支持规则表达式。
 *      在tb_auth_rule 表中定义一条规则时，如果type为1， condition字段就可以定义规则表达式。 如定义{score}>5  and {score}<100  表示用户的分数在5-100之间时这条规则才会通过。
 * 
...
class AuthRules extends Base
{
    ...
    protected $_config = array(
        ...
        'auth_user' => 'tb_members'//用户信息表
    );
    public $db;
    public function __construct() {
        if (C('auth')) {
            //可设置配置项 config.auth, 此配置项为数组。
            $this->_config = array_merge($this->_config, C('auth'));
        }
        $this->db = db();
    }
    ...
    public function getGroups($uid) {
        ...
        $sql = "select g.id,g.title,g.rules from ".$this->_config['auth_group_access']." as a 
                left jion ".$this->_config['auth_group']." as g on a.group_id=g.id  
                where a.uid='$uid'";
        $groups[$uid] = $this->db->select($sql);
        return $groups[$uid];
    }
    ...
    protected function getAuthList($uid) {
        ...
        //读取用户组所有权限规则(in)
        $map['id'] = array('in', $ids);
        $sql = "select `name`,`type`,`condition` from ".$this->_config['auth_rule']." where id in($ids)";
    ...
    protected function getUserInfo($uid) {
        ...
        if(!isset($userinfo[$uid])){
            $sql = "select * from ".$this->_config['auth_user']." where uid = '$uid'";
            $userinfo[$uid] = $this->db->select($sql,1);
