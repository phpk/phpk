...
xp.ui.form = function(param) {
	var Validator = {
		...
		Validate : function(theForm, mode, cls) {
			var obj = theForm || event.srcElement, count = obj.elements.length;
			this.ErrorMessage.length = 1;
			this.ErrorItem.length = 1;
			this.ErrorItem[0] = obj;
			for ( var i = 0; i < count; i++) {
				with (obj.elements[i]) {
					var _dtype = getAttribute("dtype");
					if (typeof (_dtype) == "object"
							|| typeof (this[_dtype]) == "undefined")
						continue;
	...
	el.onsubmit = function(){
		mod = parseInt(param.mod) || 3;
		cls = param.errorcls || '';
		console.log(cls);
		return Validator.Validate(el,mod,cls);
	}
